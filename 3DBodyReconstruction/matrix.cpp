...
static FLOATA sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static FLOATA maxarg1,maxarg2;
...
namespace selfMatrix
{
	...
	Matrix::Matrix(const int32_t m_, const int32_t n_, const FLOATA* val_) {
		allocateMemory(m_, n_);
		int32_t k = 0;
	...
	Matrix::Matrix(const Matrix &M) {
		...
		for (int32_t i = 0; i < M.m; i++)
			memcpy(val[i], M.val[i], M.n * sizeof(FLOATA));
	}
	...
	void Matrix::getData(FLOATA* val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
		if (i2 == -1) i2 = m - 1;
		if (j2 == -1) j2 = n - 1;
		int32_t k = 0;
	...
	void Matrix::setVal(FLOATA s, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
		if (i2 == -1) i2 = m - 1;
		if (j2 == -1) j2 = n - 1;
		if (i2 < i1 || j2 < j1) {
	...
	void Matrix::setDiag(FLOATA s, int32_t i1, int32_t i2) {
	...
	Matrix Matrix::rotMatX(const FLOATA &angle) {
		FLOATA s = sin(angle);
		FLOATA c = cos(angle);
		Matrix R(3, 3);
		R.val[0][0] = +1;
	...
	Matrix Matrix::rotMatY(const FLOATA &angle) {
		FLOATA s = sin(angle);
		FLOATA c = cos(angle);
		Matrix R(3, 3);
		R.val[0][0] = +c;
	...
	Matrix Matrix::rotMatZ(const FLOATA &angle) {
		FLOATA s = sin(angle);
		FLOATA c = cos(angle);
		Matrix R(3, 3);
		R.val[0][0] = +c;
	...
	Matrix Matrix::operator* (const FLOATA &s) {
		Matrix C(m, n);
		for (int32_t i = 0; i < m; i++)
			for (int32_t j = 0; j < n; j++)
				C.val[i][j] = val[i][j] * s;
	...
	bool Matrix::solve(const Matrix &M, FLOATA eps) {
	...
	bool Matrix::lu(int32_t *idx, FLOATA &d, FLOATA eps) {
		...
		int32_t i, imax, j, k;
		FLOATA   big, dum, sum, temp;
		FLOATA* vv = (FLOATA*)malloc(n * sizeof(FLOATA)); // vv stores the implicit scaling of each row.
		d = 1.0;
	...
	void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
		...
		FLOATA* w = (FLOATA*)malloc(n * sizeof(FLOATA));
		FLOATA* rv1 = (FLOATA*)malloc(n * sizeof(FLOATA));

		int32_t flag, i, its, j, jj, k, l, nm;
		FLOATA   anorm, c, f, g, h, s, scale, x, y, z;
		...
		for (k = n - 1; k >= 0; k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
			for (its = 0; its < 30; its++) { // and over allowed iterations.
				...
				for (l = k; l >= 0; l--) { // Test for splitting.
					nm = l - 1;
					if ((FLOATA)(fabs(rv1[l]) + anorm) == anorm) { flag = 0; break; }
					if ((FLOATA)(fabs(w[nm]) + anorm) == anorm) { break; }
				}
				if (flag) {
					...
					for (i = l; i <= k; i++) {
						f = s*rv1[i];
						rv1[i] = c*rv1[i];
						if ((FLOATA)(fabs(f) + anorm) == anorm) break;
						g = w[i];
						h = pythag(f, g);
						w[i] = h;
						h = 1.0 / h;
		...
		int32_t s2, inc = 1;
		FLOATA   sw;
		FLOATA* su = (FLOATA*)malloc(m * sizeof(FLOATA));
		FLOATA* sv = (FLOATA*)malloc(n * sizeof(FLOATA));
	...
	/************************************************************************/
	fstream& operator<< (fstream& out, const Matrix& M) {
		if (M.m == 0 || M.n == 0) {
			out << "[empty matrix]";
		}
		else {
	...
	void Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
		...
		val = (FLOATA**)malloc(m * sizeof(FLOATA*));
		val[0] = (FLOATA*)calloc(m*n, sizeof(FLOATA));
	...
	FLOATA Matrix::pythag(FLOATA a, FLOATA b) {
		FLOATA absa, absb;
		absa = fabs(a);
		absb = fabs(b);
		if (absa > absb)
