...
//typedef double FLOATA;      // double precision
typedef float  FLOATA;    // single precision
namespace selfMatrix
{
	...
	public:
		...
		Matrix(const int32_t m, const int32_t n, const FLOATA* val_); // init mxn matrix with values from array 'val'
		...
		void getData(FLOATA* val_, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);
		...
		void setVal(FLOATA s, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);

		// set (part of) diagonal to scalar, -1 as end replaces whole diagonal
		void setDiag(FLOATA s, int32_t i1 = 0, int32_t i2 = -1);
		...
		static Matrix rotMatX(const FLOATA &angle);
		static Matrix rotMatY(const FLOATA &angle);
		static Matrix rotMatZ(const FLOATA &angle);
		...
		Matrix  operator* (const FLOATA &s);  // multiply with scalar
		Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
		Matrix  operator/ (const FLOATA &s);  // divide by scalar
		Matrix  operator- ();                // negative matrix
		Matrix  operator~ ();                // transpose
		FLOATA   l2norm();                   // euclidean norm (vectors) / frobenius norm (matrices)
		FLOATA   mean();                     // mean of all elements in matrix
		...
		FLOATA  det();                                             // returns determinant of matrix
		bool   solve(const Matrix &M, FLOATA eps = 1e-20);            // solve linear system M*x=B, replaces *this and M
		bool   lu(int32_t *idx, FLOATA &d, FLOATA eps = 1e-20);        // replace *this by lower upper decomposition
	...
	private:
		...
		void releaseMemory();
		inline FLOATA pythag(FLOATA a, FLOATA b);
