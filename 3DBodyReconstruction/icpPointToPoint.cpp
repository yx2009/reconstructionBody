...
double IcpPointToPoint::fitStep (double *T,const int32_t T_num,selfMatrix::Matrix &R,selfMatrix::Matrix &t,const std::vector<int32_t> &active) {
  ...
  // init mean
  selfMatrix::Matrix mu_m(1,dim);
  selfMatrix::Matrix mu_t(1,dim);
  ...
  selfMatrix::Matrix q_m = p_m - selfMatrix::Matrix::ones(active.size(),1)*mu_m;
  selfMatrix::Matrix q_t = p_t - selfMatrix::Matrix::ones(active.size(),1)*mu_t;

  // compute relative rotation matrix R and translation vector t
  selfMatrix::Matrix H = ~q_t*q_m;
  selfMatrix::Matrix U,W,V;
  H.svd(U,W,V);
  selfMatrix::Matrix R_ = V*~U;
  selfMatrix::Matrix t_ = ~mu_m - R_*~mu_t;
  ...
  if (dim==2) return max((R_-selfMatrix::Matrix::eye(2)).l2norm(),t_.l2norm());
  else        return max((R_-selfMatrix::Matrix::eye(3)).l2norm(),t_.l2norm());
}

std::vector<int32_t> IcpPointToPoint::getInliers(double *T, const int32_t T_num, const selfMatrix::Matrix &R, const selfMatrix::Matrix &t, const double indist) {
...
double IcpPointToPoint::averLength(double *T, const int32_t T_num, const selfMatrix::Matrix &R, const selfMatrix::Matrix &t, const double indist) {

	// init inlier vector + query point + query result
	//vector<int32_t>            inliers;
	std::vector<float>         query(dim);
	kdtree::KDTreeResultVector neighbor;
	vector<double> avlength;
	// dimensionality 2
	if (dim == 2) {
		...
		for (int32_t i = 0; i < T_num; i++) {
			...
			// check if it is an inlier
			if (indist>0&&neighbor[0].dis < indist)
				avlength.push_back(neighbor[0].dis);
			else if (indist<0)
			{
				avlength.push_back(neighbor[0].dis);
			}
	...
	double sumr = std::accumulate(std::begin(avlength), std::end(avlength), 0.0);
	// return vector with inliers
	return sumr/avlength.size();
}
